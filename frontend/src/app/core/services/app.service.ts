...
export class AppService {
  ...
  updateSalePaymentMethod(saleId: string, paymentId: string, paymentMethod: string): Observable<Sale> {
    return this.http.put<any>(`${this.API_BASE_URL}/sales/${saleId}/payments/${paymentId}/method`, {
      paymentMethod
    })
      .pipe(
        map(apiSale => this.convertApiSaleToSale(apiSale)),
  ...
  deleteSale(saleId: string): Observable<any> {
    return this.http.delete(`${this.API_BASE_URL}/sales/${saleId}`)
      .pipe(
        tap(() => {
          ...
          this.updateAppState({
            ...currentState,
            sales: currentState.sales.filter(s => s.id !== saleId)
          });

          // Refresh sales to ensure consistency
          this.loadSales().subscribe();
        }),
        catchError(error => {
          return throwError(() => error);
